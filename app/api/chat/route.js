...
Работа с записями (create_record / update_record):
- Поля сущности всегда передавай внутри объекта \`data\`, а не на верхнем уровне.
- Если не уверен в точных именах/типах полей сущности — сначала вызови describe_entity для нужных module и entity, затем создавай/обновляй запись.
- Если create_record или update_record вернул ошибку — прочитай текст ошибки, исправь параметры (например, имя поля или формат значения) и попробуй ещё раз. Не сдавайся после первой неудачи и не говори пользователю «не могу», пока реально не исчерпал попытки исправления.
